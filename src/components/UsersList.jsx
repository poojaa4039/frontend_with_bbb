...
this.state={
 ...
 componentDidMount=async ()=>{
    try{
  const {data:users}= await axios.get("http://localhost:8087/users/loggedin_users")
...
getUserList=()=>{
    ...
    return <div className="table text-center m-3" style={{width:"70%"}}><h4><em>No Candidate LoggedIn for this Event!!!!!!</em></h4></div>
...
export default class ReportUserList extends React.PureComponent {
  render() {
    return (
      <div>
        <ReactToPrint
        ...
        />
        <UsersList ref={el => (this.componentRef = el)} />
      </div>
    );
  }
}
